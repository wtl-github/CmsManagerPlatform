							...
							<c:forEach items="${communityList}" var="community"
								...
								<tr class="main_info">
									<%-- <td><input type="checkbox" name="uploadIds"
										value="${community.id }" lay-skin="primary" lay-filter="choose"/></td> --%>
									<td style="display: none">${community.id }</td>
									<td>${community.subdistrictName }</td>
									<td>${community.subdistrictAddress }</td>
									<td>${community.subdistrictTel }</td>
									<td>
										<button type="button"
        ...
        var action={
       		see:function () {
                   ...
                   layer.open({
                       ...
                       content: '<div style="padding:15px">'
                               + '<table class="layui-table">'
                               + '<tr><td>社区名称</td><td>' +that.find('td:nth-child(2)').text() + '</td></tr>'
                               + '<tr><td>社区地址</td><td>' + that.find('td:nth-child(3)').text() + '</td></tr>'
                               + '<tr><td>社区电话</td><td>' + that.find('td:nth-child(4)').text() + '</td></tr>'
                               + '</table></div>',
                       btn: ['返回'],
                       btnAlign: 'c'
                   });
            ...
            edit:function(){
                ...
                layer.open({
                      ...
                      area:['610px','300px'],
                      maxmin:true,
                     content:'/community/edit.html?id='+that.find("td:nth-child(1)").text()
                })
            },
			del:function(){
                var that=$(this).parent().parent();
